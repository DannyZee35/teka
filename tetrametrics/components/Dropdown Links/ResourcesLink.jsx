...
const ResourcesLink = ({ResourcesClick}) => {
      ...
      <div ref={dropdownRef} className="absolute mt-4 bg-white text-black border p-10 w-[800px] border-gray-300 rounded-md block left-1/4 transform -translate-x-1/4">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            Read the latest in ecommerce news, education, and data trends            </p>
          </div>
          <div className="flex flex-col">
            <span className="flex items-center gap-3">
              <Link href={"/blogs"} onMouseEnter={() => setBlogIconHovered(true)} onClick={ResourcesClick} onMouseLeave={() => setBlogIconHovered(false)}>Blog</Link>
              <div className={`${BlogIconHovered ? 'translate-x-2 transition ease-out delay-75' : 'transition ease-in delay-75'}`}>
                <FaArrowRightLong />
              </div>
            </span>
            <span className="flex items-center gap-3">
              <Link href={"/blogs"} onMouseEnter={() => setNewsIconHovered(true)} onClick={ResourcesClick} onMouseLeave={() => setNewsIconHovered(false)}>Newsletter</Link>
